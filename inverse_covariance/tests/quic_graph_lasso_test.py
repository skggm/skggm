...
class TestQuicGraphLasso(object):
    @pytest.mark.parametrize("params_in, expected", [
        ...
        ({
            'lam': 0.5,
            'mode': 'trace',
        }, [3.2437533337151625, 3.4490549523890652, 32.290292419357321, 0.21836515326396364]),  # NOQA
        ({
            'lam': 0.5,
            'mode': 'path',
            'path': np.array([1.0, 0.9, 0.8, 0.7, 0.6, 0.5]),
        }, [8.3256240637201717, 9.7862122341861983, 22.752074276274861, 1.6530965731149066e-08]),  # NOQA
        ({
            'lam': 1.0,
            'max_iter': 100,
            'init_method': 'cov',
    ...
    def test_integration_quic_graph_lasso(self, params_in, expected):
        ...
        print(result_vec)
        assert_allclose(expected, result_vec, atol=1e-1)
    ...
    @pytest.mark.parametrize("params_in, expected", [
        ...
        ({'lam': 0.5 * np.ones((10, 10)) - 0.5 * np.diag(np.ones((10,))),
          'n_refinements': 1}, [4.6765, 49.24459, 3.26151, 6.769744583801085e-07]),  # NOQA
        ({'lam': 0.5 * np.ones((10, 10)) - 0.5 * np.diag(np.ones((10,))),
          'n_refinements': 1,
    ...
    @pytest.mark.parametrize("params_in, expected", [
        ...
        ({'lam': 0.5 * np.ones((10, 10)), 'init_method': custom_init}, [0.0106, 35056.88460]),  # NOQA
    ])
